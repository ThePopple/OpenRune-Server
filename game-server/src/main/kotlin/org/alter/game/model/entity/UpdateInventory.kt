...
import net.rsprot.protocol.common.game.outgoing.inv.InventoryObject.invoke
...
public object UpdateInventory {
    ...
    public fun updateInvStopTransmit(player: Player, inv: Inventory) {
        player.write(UpdateInvStopTransmit(inv.type.id))
    }
...
private class RspObjProvider(private val objs: Array<Item?>) : UpdateInvFull.ObjectProvider {
    override fun provide(slot: Int): Long {
        val obj = objs.getOrNull(slot) ?: return InventoryObject.NULL
        return InventoryObject(slot, obj.id, obj.amount)
    }
}

private class RspIndexedObjProvider(private val objs: Array<Item?>, updateSlots: Iterator<Int>) :
